...
That is for a given set $A$ and a comparison operator, the two valid outputs are either the original set $A$ or undefined ($\bot$). 
...
S =
\begin{cases}
A, & \text{if } A \subseteq B \\
\bot, & \text{otherwise}
\end{cases}
$$

For example, given the sets $`\{1, 2, 3\}`$ and $`\{1, 2\}`$ the subset check of $`\{1, 2, 3\} \subseteq \{1, 2\}`$ will evaluate to $\bot$ which will cause a type error. 
...
For example, given the sets $`\{1, 2\}`$ and $`\{1, 2\}`$ the strict subset check of $`\{1, 2\} \subset \{1, 2\}`$ will evaluate to $\bot$, which will cause a type error.
