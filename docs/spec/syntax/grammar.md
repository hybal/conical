...
> **NOTE**:
> TYPE_INTERFACE_SUGAR and TYPE_IMPL_SUGAR may be merged in the future

```ebnf

TYPE_EXPRESSION                  ::= TYPE_EXPRESSION_METADATA

TYPE_EXPRESSION_METADATA         ::= TYPE_EXPRESSION_PREFIX ( KEYWORD_WITH TYPE_EXPRESSION_PREFIX )*

TYPE_EXPRESSION_PREFIX           ::= KEYWORD_REL TYPE_EXPRESSION | TYPE_EXPRESSION_DEFAULT

TYPE_EXPRESSION_DEFAULT          ::= TYPE_EXPRESSION_INCLUSION { '=' EXPRESSION }
...
TYPE_EXPRESSION_PRODUCT          ::= TYPE_EXPRESSION_UNARY ( '*' TYPE_EXPRESSION_UNARY )*

TYPE_EXPRESSION_UNARY            ::= ( '&' | '[]' | '[' EXPRESSION ']' )* TYPE_EXPRESSION_GROUPING

TYPE_EXPRESSION_GROUPING         ::= '(' TYPE_EXPRESSION ')' 
                                   | EXPRESSION_BLOCK 
                                   | IDENTIFIER 
...
FUNCTION_DECLARATION_INLINE     ::= FUNCTION_HEADER '(' FUNCTION_PARAMETER_LIST_INLINE ')'
                                    {'->' TYPE_EXPRESSION}

FUNCTION_DECLARATION_POSTFIX    ::= FUNCTION_HEADER '(' FUNCTION_PARAMETER_LIST_POSTFIX ')'
...
> **NOTE**:
> The precedence level of type expressions are subject to change.

```ebnf
EXPRESSION                ::= KEYWORD_TYPE TYPE_EXPRESSION 
                            | KEYWORD_IMPORT EXPRESSION
                            | EXPRESSION_RETURN

EXPRESSION_RETURN         ::= KEYWORD_RETURN EXPRESSION | EXPRESSION_MATCH

EXPRESSION_BLOCK          ::= '{' (STATEMENT)* '}'
...
EXPRESSION_MATCH          ::= KEYWORD_MATCH EXPRESSION '{' MATCH_ARM+ '}' | EXPRESSION_LOGICAL_OR

MATCH_ARM                 ::= TYPE_EXPRESSION '=>' { '|' IDENTIFIER ( ',' IDENTIFIER ) {','} '|' } EXPRESSION_OPTIONAL_BLOCK
...
EXPRESSION_INITIALIZER    ::= '.' { '(' TYPE_EXPRESSION ')' } '{' ({'.' IDENTIFIER '='} EXPRESSION) (',' {'.' IDENTIFIER '='} EXPRESSION)* {','} '}'
                            | EXPRESSION_PRIMARY

EXPRESSION_PRIMARY        ::= '(' EXPRESSION ')'
                            | EXPRESSION_LAMBDA
                            | EXPRESSION_BLOCK 
                            | EXPRESSION_COMPTIME_BLOCK
                            | EXPRESSION_PURE_BLOCK
                            | EXPRESSION_INLINE_BLOCK
                            | EXPRESSION_TERMINAL

EXPRESSION_COMPTIME_BLOCK ::= KEYWORD_COMPTIME EXPRESSION_OPTIONAL_BLOCK

EXPRESSION_INLINE_BLOCK   ::= KEYWORD_INLINE EXPRESSION_OPTIONAL_BLOCK

EXPRESSION_PURE_BLOCK     ::= KEYWORD_PURE EXPRESSION_OPTIONAL_BLOCK

LAMBDA_PARAMETER_LIST_INLINE  ::= {GENERIC_LIST} { {BINDING_MODIFIER} IDENTIFIER {':' TYPE_EXPRESSION} (',' {BINDING_MODIFIER} IDENTIFIER {':' TYPE_EXPRESSION} )* {','}}

LAMBDA_PARAMETER_LIST_POSTFIX ::= {GENERIC_LIST} { {BINDING_MODIFIER} IDENTIFIER (',' {BINDING_MODIFIER} IDENTIFIER) {','}}

LAMBDA_INLINE     ::= '\\' '(' LAMBDA_PARAMETER_LIST_INLINE ')'
                      {'->' TYPE_EXPRESSION}

LAMBDA_POSTFIX    ::= '\\' '(' LAMBDA_PARAMETER_LIST_POSTFIX ')'
                      { ':' (TYPE_EXPRESSION | '_' ) | '(' ( TYPE_EXPRESSION | '_' ) (',' (TYPE_EXPRESSION | '_') )* {','} ')'} 
                      { '->' TYPE_EXPRESSION }

EXPRESSION_LAMBDA         ::= '\\' (GENERIC | {BINDING_MODIFIER} IDENTIFIER {':' TYPE_EXPRESSION} { '->' TYPE_EXPRESSION } 
                            | LAMBDA_INLINE
                            | LAMBDA_POSTFIX
...
```ebnf
STATEMENT             ::= (ATTRIBUTE)* (ITEM | ASSIGNMENT | LOOP | TERMINATED_EXPRESSION | EXPRESSION)

TERMINATED_EXPRESSION ::= EXPRESSION ';'

ITEM                  ::= (ATTRIBUTE)* ITEM_MODIFIERS (LET_BINDING | FUNCTION_DECLARATION | TYPE_DECLARATION) | USE_EXPRESSION

USE_EXPRESSION        ::= KEYWORD_USE EXPRESSION


ITEM_MODIFIERS        ::= {VISIBILITY} {LINKAGE}

VISIBILITY            ::= KEYWORD_PUB

LINKAGE               ::= KEYWORD_EXTERN | KEYWORD_EXPORT
